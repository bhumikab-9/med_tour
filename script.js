...
/* =========================================
   HOMEPAGE — TRUSTED HOSPITALS
   (Loaded live from the MySQL-backed API)
========================================= */

function loadTrustedHospitals() {

    var grid = document.getElementById("trustedHospitalGrid");
    if (!grid) return;

    fetch("http://localhost:5000/api/hospitals?limit=3&sort=rating_desc")
        .then(function (res) { return res.json(); })
        .then(function (data) {

            var hospitals = (data && data.hospitals) || [];

            if (!hospitals.length) {
                grid.innerHTML = '<p style="grid-column:1/-1;text-align:center;color:var(--muted);">No hospitals found yet.</p>';
                return;
            }

            var imageClasses = ["hospital-one", "hospital-two", "hospital-three"];

            grid.innerHTML = hospitals.map(function (h, i) {
                var rating = h.rating ? Number(h.rating).toFixed(1) : "New";
                var city = h.city_state || "India";
                var imgClass = imageClasses[i % imageClasses.length];
                var blurb = (h.specialities && h.specialities.length)
                    ? h.specialities.slice(0, 2).join(" & ") + " care."
                    : "Multi-specialty healthcare and advanced medical care.";

                return (
                    '<article class="hospital-card">' +
                        '<div class="hospital-image ' + imgClass + '"><span>Hospital</span></div>' +
                        '<div class="hospital-info">' +
                            '<div class="hospital-title"><h3>' + escapeHtml(h.name) + '</h3><span class="verified">\u2713</span></div>' +
                            '<div class="hospital-meta">\u2B50 ' + rating + ' &nbsp; \u2022 &nbsp; ' + escapeHtml(city) + '</div>' +
                            '<p>' + escapeHtml(blurb) + '</p>' +
                            '<button onclick="window.location.href=\'hospital-details.html?id=' + h.id + '\'">View Hospital \u2192</button>' +
                        '</div>' +
                    '</article>'
                );
            }).join("");
        ...
        .catch(function (err) {
            grid.innerHTML = '<p style="grid-column:1/-1;text-align:center;color:red;">Could not load hospitals. Is the server running?</p>';
            console.error(err);
        });
}
...
const treatmentDatabase = {
    ...
    "Fracture Treatment": {
        ...
        description:
            "Diagnosis and treatment of broken bones, from casting to surgical fixation.",
        ...
        overviewText1:
            "Fracture treatment realigns and stabilizes a broken bone so it can heal correctly, using casts, splints, or surgical implants like plates, screws or rods depending on severity.",

        overviewText2:
            "Orthopedic specialists assess the fracture with imaging and choose the least invasive option that still restores strength and mobility.",

        procedureType: "Surgical / Non-Surgical",

        faq1:
            "How much does fracture treatment cost in India?",

        faq2:
            "How long until I can walk or use the limb normally again?"
    },
    ...
    "MRI Scan": {
        ...
        description:
            "Detailed imaging of internal organs and tissues using magnetic resonance.",
        ...
        overviewText1:
            "An MRI (Magnetic Resonance Imaging) scan uses magnetic fields and radio waves to create detailed images of organs, soft tissue, bone and other internal structures.",

        overviewText2:
            "It's a painless, non-invasive procedure commonly used to investigate joint, brain, spine and abdominal conditions.",

        procedureType: "Diagnostic",

        faq1:
            "How much does an MRI scan cost in India?",
    ...
    "Normal Delivery": {
        ...
        description:
            "Maternity care and delivery support from experienced obstetric teams.",
        ...
        overviewTitle:
            "What does maternity & delivery care include?",

        overviewText1:
            "Maternity care covers prenatal monitoring, delivery (normal or cesarean) and postnatal support for mother and baby, guided by an obstetric team.",

        overviewText2:
            "Hospitals offering delivery care provide labor rooms, neonatal support and postnatal recovery facilities tailored to individual birth plans.",

        procedureType: "Medical / Surgical",

        faq1:
            "How much does delivery cost in India?",

        faq2:
            "What's the difference between normal delivery and a C-section?"
    },
    ...
    "Hair Transplant": {
        ...
        description:
            "Modern hair restoration procedures performed by specialists.",
        ...
        overviewText1:
            "A hair transplant moves hair follicles from a donor area to thinning or balding areas, most commonly using FUE (Follicular Unit Extraction) technique.",

        overviewText2:
            "Results appear gradually as transplanted follicles establish new growth cycles over several months.",

        procedureType: "Surgical / Cosmetic",
...
document.addEventListener(
    ...
    function () {
        ...
        */
        ...
        /*
           Homepage — Trusted Hospitals (live from MySQL)
        */

        loadTrustedHospitals();
